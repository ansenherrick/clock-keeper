...
const { buildCsv, buildExportFileName } = require('../lib/csv');
...
router.post('/exports', requireUser, async (req, res, next) => {
  try {
    ...
    const filename = buildExportFileName(req.user.name, exportType, exportedAt);
    const csv = buildCsv(hydratedShifts, req.user, exportedAt, exportType);

    for (const shift of hydratedShifts) {
      await pool.query(
        'INSERT INTO shift_exports (shift_id, batch_id, exported_at, type) VALUES ($1, $2, $3, $4)',
        [shift.id, batchId, exportedAt, exportType],
      );
    }
